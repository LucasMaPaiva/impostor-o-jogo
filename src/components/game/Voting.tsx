  ...
  return (
    <div className="flex-1 flex flex-col space-y-8">
      ...
      <div className="max-h-[340px] overflow-y-auto pr-1 flex flex-col gap-2 custom-scrollbar">
        {room.players.map((p) => (
          ...
          >
            ...
            {/* Votos Recebidos */}
            {room.players.some(voter => voter.votedFor === p.id) && (
              <div className="px-4 pb-3 -mt-2">
                <div className="flex flex-wrap gap-1.5 pt-2 border-t border-zinc-800/50">
                  {room.players.filter(voter => voter.votedFor === p.id).map(voter => (
                    <motion.div
                      key={voter.id}
                      initial={{ scale: 0.8, opacity: 0 }}
                      animate={{ scale: 1, opacity: 1 }}
                      className="text-[10px] font-black uppercase tracking-tight px-2 py-0.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-500"
                    >
                      {voter.name}
                    </motion.div>
                  ))}
      ...
      <motion.button
        ...
        className="w-full mt-4 p-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 hover:bg-emerald-500/10 transition-all group flex flex-col gap-3 shrink-0"
      >
        ...
        {room.players.some(voter => voter.votedFor === "RE_ROUND") && (
          <div className="w-full flex flex-wrap gap-1.5 pt-2 border-t border-emerald-500/10">
            {room.players.filter(voter => voter.votedFor === "RE_ROUND").map(voter => (
              <div key={voter.id} className="text-[9px] font-black uppercase px-2 py-0.5 rounded-lg bg-emerald-500/20 border border-emerald-500/30 text-emerald-400">
                {voter.name}
              </div>
            ))}
          </div>
